...
export default function PaymentWorkbench({rows,role,search,setSearch,open,create,onDelete}:{rows:Payment[];role:string;search:string;setSearch:(s:string)=>void;open:(p:Payment)=>void;create:()=>void;onDelete?:(p:Payment)=>void}){
 const[tab,setTab]=useState("All"),[department,setDepartment]=useState("All departments");
 ...
 const download=()=>{
  ...
  csv([head,...body,...foot],
    `payments-${slug(queue)}${department==="All departments"?"":"-"+slug(department)}-${new Date().toISOString().slice(0,10)}.csv`);
 };
