...
export default function RequestorWorkspace({rows,open,create}:{rows:P[];open:(p:P)=>void;create:()=>void}){
  ...
  /* Clamped rather than used as given: if the list shortens under a reader who has paged
     forward - a filter changes, a request is withdrawn - an unclamped offset would land
     past the end and show an empty table, which reads as "no requests" rather than as a
     stale page. */
  const start=Math.min(offset,Math.max(0,Math.floor((rows.length-1)/PER_PAGE)*PER_PAGE));
  const mine=rows.slice(start,start+PER_PAGE);
  /* The table pages ten at a time; the report covers every request they have raised.
     A download that quietly stopped at one page would be read as the whole record. */
  const download=()=>csv([
  ...
  return <div className="page rq-page">
    ...
    <section className="panel table-panel rq-table">
      ...
      {/* keyed off the whole register, not the page: an empty page is a paging fault,
          and telling a requestor with twenty requests that they have none is worse. */}
      ...
      <Pager total={rows.length} limit={PER_PAGE} offset={start} setOffset={setOffset}/>
    </section>
  </div>}
