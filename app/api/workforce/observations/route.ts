...
export async function POST(req:Request){
  try{
    ...
    /* Everyone tagged has to answer, so everyone tagged is told. */
    await notify(await emailsForEmployees(tagged),{
      title:`You were tagged on an observation: ${row.title}`,
      body:[row.risk?`${row.risk} risk`:"",row.target?`answer by ${row.target}`:""].filter(Boolean).join(" · "),
      module:"observations",recordId:id},actor?.email);
...
export async function PATCH(req:Request){
  try{
    ...
    await writeWithAudit(statements,actorOf(req,body),"observation",id,
      ...
      `${row.ref} · ${row.status}`);
    /* A resolution or a reopening goes to everyone tagged and to whoever raised it. */
    if(action==="resolve"||action==="reopen"){
      const tagRows=await db.select({e:wfObsTags.employeeId}).from(wfObsTags).where(eq(wfObsTags.observationId,id));
      await notify([existing.raisedByEmail||"",...await emailsForEmployees(tagRows.map(t=>t.e))],{
        title:action==="resolve"?`Observation resolved: ${row.title}`:`Observation reopened: ${row.title}`,
        body:action==="resolve"?row.resolution||"":"",module:"observations",recordId:id},actor?.email);
    }
