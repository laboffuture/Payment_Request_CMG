...
export default function Home(){
 ...
 {active==="specialaudit"&&<AuditTaskQueue title="Special audit tasks" kind="Special Audit" tasks={auditTasks} role={role} accept={acceptAuditTask} update={updateAuditTask} openImport={()=>setActive("imports")}/>} 
 {active==="observations"&&<Observations/>}
 {active==="community"&&<CommunityChat user={userName} flash={flash}/>}
 {active==="meetings"&&<AuditTaskQueue title="Meeting tasks" kind="Meeting" tasks={auditTasks} role={role} accept={acceptAuditTask} update={updateAuditTask} openImport={()=>setActive("imports")}/>} 
...
function PaymentForm({close,added,companies}:{close:()=>void;added:(p:Payment)=>void;companies:{id:string;name:string}[]}){const[v,setV]=useState({company:companies[0]?.name||"",vendor:"",amount:"",currency:"AED",department:"Projects",due:"2026-08-30",description:""});const[files,setFiles]=useState<File[]>([]);const[saving,setSaving]=useState(false);const submit=async(e:any)=>{e.preventDefault();if(saving)return;setSaving(true);let p:any={...v,amount:Number(v.amount),requestNo:`PAY-2026-${1050+Math.floor(Math.random()*100)}`,status:"Submitted",owner:"Accountant queue",urgency:"Normal",id:Date.now()};try{let r=await fetch("/api/payments",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(p)});if(r.ok)p=((await r.json()) as {payment:Payment}).payment}catch{}/* The documents were collected and then dropped: the input had no handler, so nothing
...
for(const file of files){try{const dataUrl=await asDataUrl(file);await fetch("/api/attachments",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({entityType:"payment",entityId:String(p.id),kind:"Other",fileName:file.name,dataUrl})});}catch{}}setSaving(false);added(p)};return <><button className="overlay" onClick={close}/><form className="modal" onSubmit={submit}><header><div><small>NEW REQUEST</small><h2>Create payment request</h2></div><button type="button" onClick={close}><X/></button></header><div className="form">{[["Company","company"],["Department","department"],["Vendor / beneficiary","vendor"],["Amount","amount"],["Due date","due"]].map(([label,key])=><label className={key==="vendor"?"wide":""} key={key}>{label}{key==="company"?<select value={v.company} onChange={e=>setV({...v,company:e.target.value})}>{companies.map(c=><option key={c.id}>{c.name}</option>)}</select>:key==="department"?<select value={v.department} onChange={e=>setV({...v,department:e.target.value})}><option>Projects</option><option>Procurement</option><option>Finance</option><option>Operations</option></select>:<input required type={key==="amount"?"number":key==="due"?"date":"text"} value={(v as any)[key]} onChange={e=>setV({...v,[key]:e.target.value})}/>}</label>)}<label>Currency<select value={v.currency} onChange={e=>setV({...v,currency:e.target.value})}><option>AED</option><option>SAR</option><option>INR</option><option>USD</option></select></label><label className="wide">Description<textarea value={v.description} onChange={e=>setV({...v,description:e.target.value})}/></label><label className="upload wide"><FileText/> Attach supporting documents<input type="file" multiple accept="image/*,application/pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.zip" onChange={e=>setFiles(Array.from(e.target.files||[]))}/>{files.length>0&&<small className="upload-list">{files.length} file{files.length===1?"":"s"}: {files.map(f=>f.name).join(", ")}</small>}</label></div><footer><button type="button" onClick={close}>Cancel</button><button className="primary" disabled={saving}>{saving?"Submitting…":"Submit to accountant"}</button></footer></form></>}
