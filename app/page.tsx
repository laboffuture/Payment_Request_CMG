...
import {AlertTriangle,Building2,CalendarClock,CalendarDays,CheckCircle2,ChevronDown,CircleDollarSign,ClipboardCheck,FileBarChart,FileText,History,Import,LayoutDashboard,LogOut,Menu,MessageSquareText,Plus,ReceiptText,Search,Settings,ShieldCheck,SlidersHorizontal,Users,X,GraduationCap,Upload} from "lucide-react";
...
export default function Home(){
 ...
 useEffect(()=>{if(!userEmail)return;
   fetch("/api/settings/options").then(r=>r.json()).then((d:any)=>{
     ...
     setTdsChoices(on("payment.tds"))}).catch(()=>{});
   fetch("/api/settings/fields").then(r=>r.json()).then((d:any)=>
     setExtraFields((d.fields||[]).filter((f:any)=>f.form==="payment"&&f.active)))
     .catch(()=>{})},[userEmail,masterVersion]);
 ...
 {active==="settings"&&<SettingsDesk changed={()=>setMasterVersion(n=>n+1)}/>}
...
function PaymentForm({close,added,companies,departments,natures,currencies,tdsChoices,extraFields}:{close:()=>void;added:(p:Payment)=>void;companies:{id:string;name:string}[];departments:string[];natures:string[];currencies:string[];tdsChoices:string[];extraFields:{id:string;label:string;type:string;options:string;required:number}[]}){const[v,setV]=useState({company:companies[0]?.name||"",vendor:"",amount:"",currency:currencies[0]||"",department:departments[0]||"",due:"2026-08-30",description:"",poNumber:"",nature:natures[0]||"",tds:"No"});const[extra,setExtra]=useState<Record<string,string>>({});const[files,setFiles]=useState<File[]>([]);const[saving,setSaving]=useState(false);useEffect(()=>{if(natures.length&&!natures.includes(v.nature))
 ...
 useEffect(()=>{if(tdsChoices.length&&!tdsChoices.includes(v.tds))
  setV(c=>({...c,tds:tdsChoices[0]}))},[tdsChoices,v.tds]);useEffect(()=>{if(departments.length&&!departments.includes(v.department))setV(c=>({...c,department:departments[0]}))},[departments,v.department]);const submit=async(e:any)=>{e.preventDefault();if(saving)return;setSaving(true);const filled=extraFields.filter(x=>(extra[x.id]||"").trim()!=="")
   .map(x=>({id:x.id,label:x.label,value:extra[x.id]}));
 let p:any={...v,extra:filled.length?JSON.stringify(filled):"",amount:Number(v.amount),requestNo:`PAY-2026-${1050+Math.floor(Math.random()*100)}`,status:"Submitted",owner:"Accountant queue",urgency:"Normal",id:Date.now()};try{let r=await fetch("/api/payments",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(p)});if(r.ok)p=((await r.json()) as {payment:Payment}).payment}catch{}/* The documents were collected and then dropped: the input had no handler, so nothing
