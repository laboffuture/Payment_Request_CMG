...
type Payment={nature?:string;poNumber?:string;resubmitNote?:string;resubmittedAt?:string;rejectionNote?:string;rejectedBy?:string;rejectedAt?:string;raisedBy?:string;id:number;requestNo:string;company:string;vendor:string;amount:number;currency:string;due:string;urgency:string;status:string;owner:string;department:string};
...
export default function Home(){
 ...
 const loadAuditTasks=async()=>{try{
   ...
   setAuditTasks(d.tasks.map(t=>({id:t.id,title:t.title,company:name.get(t.companyId)||t.companyId||"—",
     ...
     due:t.due,assignedTo:t.assignedTo,plannedStart:t.plannedStart,plannedEnd:t.plannedEnd,
 ...
 /* Meetings and audit tasks are the same record with a different kind, so one handler
    serves every queue. */
 const createAuditTask=async(kind:AuditTask["kind"],t:{title:string;department:string;companyId:string;due:string;notes:string})=>{
...
function Observations(){
 /* The register with real data lives on the Observations screen in the workforce
    module (/api/workforce/observations). This entry stays so the menu item keeps
    working, and points at it rather than showing invented rows. */
 return <div className="page"><Intro eyebrow="CONTROL EXCEPTIONS" title="Observations register"
   sub="Raise and answer observations from the Observations screen, where they are saved and shared."/>
  <section className="panel"><p className="queue-empty">No observations recorded here yet.</p></section></div>}
...
function PaymentForm({close,added,companies,departments}:{close:()=>void;added:(p:Payment)=>void;companies:{id:string;name:string}[];departments:string[]}){const[v,setV]=useState({company:companies[0]?.name||"",vendor:"",amount:"",currency:"AED",department:departments[0]||"",due:"2026-08-30",description:"",poNumber:"",nature:"Vendor Payment"});const[files,setFiles]=useState<File[]>([]);const[saving,setSaving]=useState(false);useEffect(()=>{if(departments.length&&!departments.includes(v.department))setV(c=>({...c,department:departments[0]}))},[departments,v.department]);const submit=async(e:any)=>{e.preventDefault();if(saving)return;setSaving(true);let p:any={...v,amount:Number(v.amount),requestNo:`PAY-2026-${1050+Math.floor(Math.random()*100)}`,status:"Submitted",owner:"Accountant queue",urgency:"Normal",id:Date.now()};try{let r=await fetch("/api/payments",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(p)});if(r.ok)p=((await r.json()) as {payment:Payment}).payment}catch{}/* The documents were collected and then dropped: the input had no handler, so nothing
