...
const tdsLine=(p:{tds?:string;tdsPercent?:string;tdsValue?:string;currency:string;amount:number})=>{
  if(p.tds!=="Yes")return p.tds==="No"?"Not applicable":"";
  const value=Number(p.tdsValue);
  return[`Yes${p.tdsPercent?` · ${p.tdsPercent}%`:""}`,
    p.tdsValue&&Number.isFinite(value)?`${p.currency} ${value.toLocaleString()}`:"",
    p.tdsValue&&Number.isFinite(value)?`Net payable ${p.currency} ${(p.amount-value).toLocaleString()}`:""]
    .filter(Boolean).join(" · ")};
...
export default function PaymentDetail({payment:p,busy=false,role,onClose,onAction,onDelete,userEmail="",companies=[],departments=[],natures=[],currencies=[],tdsChoices=[],termsChoices=[],modeChoices=[],jobs=[]}:{payment:Payment;busy?:boolean;role:string;onClose:()=>void;onAction:(s:string,note?:string,fields?:Record<string,string>)=>void;onDelete?:()=>void;userEmail?:string;companies?:{id:string;name:string}[];departments?:string[];natures?:string[];currencies?:string[];tdsChoices?:string[];termsChoices?:string[];modeChoices?:string[];jobs?:Job[]}){
 ...
 /* A query audit raised goes back to audit when it is answered - the server decides that;
    this only names it on the button. */
 const lastQuery=[...history].reverse().find(e=>e.action==="Query raised to requestor");
 const queriedByAudit=!!lastQuery&&["Pre-Audit Queue","Audit Accepted","Audit Reconfirmation"].includes(lastQuery.previousValue);
 ...
 /* The ways a request goes back. A query to the requestor returns it to be corrected and
    resubmitted; a rejection closes it for good, and a new request is raised instead. Audit
    can also put a query to accounts, who answer it and send the request back to audit.
    Each has to say why, so each button opens a remark box rather than acting immediately. */
 const SEND_BACK={
   accounts:{status:"Audit Query",label:"Query to Accounts",confirm:"Send query to Accounts",
     cls:"wf-query",ask:"What does audit need accounts to clarify?",
     hint:"Which entry, figure or document needs explaining? Accounts answer it and the request comes back to audit.",
     title:"Ask accounts a question. They answer it and the request returns to audit"},
   query:{status:"Query Raised",label:"Query to Requestor",confirm:"Send query to requestor",
     cls:"wf-query",ask:"What does the requestor need to correct?",
     hint:"Which figure is wrong, or which document is wrong or missing? The requestor corrects it and resubmits.",
     title:"Return to the requestor to correct and resubmit"},
   reject:{status:"Rejected",label:"Reject",confirm:"Reject and close",
     cls:"wf-reject",ask:"Why is this request being rejected?",
     hint:"The request is closed and cannot be resubmitted. If the payment is still needed, the requestor raises a new one.",
     title:"Close the request. The requestor must raise a new one"}} as const;
 const rejectForm=(fromAudit=false)=>{
   const offered=(fromAudit?["accounts","query","reject"]:["query","reject"]) as (keyof typeof SEND_BACK)[];
   const open=sendBack?SEND_BACK[sendBack]:null;
   return open
   ?<div className="wf-reject-form">
      <label className="wf-note">{open.ask}
        <textarea autoFocus value={remark} onChange={e=>setRemark(e.target.value)} placeholder={open.hint}/></label>
      <div className="wf-actions">
        <button onClick={()=>{setSendBack("");setRemark("")}}>Cancel</button>
        <button className={open.cls} disabled={remark.trim().length<5}
          onClick={()=>onAction(open.status,remark.trim())}>{open.cls==="wf-reject"?<X/>:<HelpCircle/>}{open.confirm}</button>
      </div>
    </div>
   :<>{offered.map(k=><button key={k} className={SEND_BACK[k].cls} onClick={()=>setSendBack(k)}
        title={SEND_BACK[k].title}>{SEND_BACK[k].cls==="wf-reject"?<X/>:<HelpCircle/>}{SEND_BACK[k].label}</button>)}</>};
 ...
 if(auditQueue&&can("Auditor"))action=<><div className="wf-callout wf-tds-seen"><ShieldCheck/>{p.tds?`TDS recorded by Accounts: ${tdsLine(p)}`:"Accounts have not recorded TDS on this request."}</div>{p.resubmitNote&&<div className="wf-success wf-reply"><Check/><div><small>CORRECTED AND RESUBMITTED</small><b>{p.resubmitNote}</b><span>{p.rejectionNote?`Returned for: ${p.rejectionNote}`:""}{p.resubmittedAt?` · ${stamp(p.resubmittedAt)}`:""}</span></div></div>}<div className="wf-callout"><ShieldCheck/>Available in the common Audit queue.</div>{noteBox("Remarks (optional)","Anything worth recording with this decision")}<div className="wf-actions">{rejectForm(true)}<button className="wf-primary" onClick={()=>onAction("Audit Accepted",withNote())}><Check/>Accept to Start</button></div></>;
 if(auditWork&&can("Auditor"))action=<><div className="wf-callout wf-tds-seen"><ShieldCheck/>{p.tds?`TDS recorded by Accounts: ${tdsLine(p)}`:"Accounts have not recorded TDS on this request."}</div>{list(auditKeys)}<label className="wf-note">Audit observation<textarea value={observation} onChange={e=>setObservation(e.target.value)}/></label><div className="wf-actions"><button className="wf-reject" onClick={()=>onAction("Observation - Audit Action",observation.trim()||undefined)}><AlertTriangle/>Raise observation to Accounts</button>{rejectForm(true)}<button className="wf-primary" disabled={!complete(auditKeys)} onClick={()=>onAction("Approved by Auditor – Ready to Release",withNote())}>Approve for release</button></div></>;
 ...
 /* Audit's question for accounts. The answer is the point, so it is required; the request
    then goes straight back to the audit queue. Accounts can still send it on to the
    requestor instead if only they can answer it. */
 if(auditQuery&&can("Accountant"))action=<><div className="wf-observation"><HelpCircle/><div>
   <small>AUDIT QUERY FOR ACCOUNTS</small><b>{p.lastActionNote||"No question was recorded."}</b>
   <span>{p.lastActionBy?`From ${p.lastActionBy}`:""}</span></div></div>
   {noteBox("Reply to audit","Answer the query - what you checked, what you found, or what you corrected")}
   <div className="wf-actions">{rejectForm()}<button className="wf-primary" disabled={stageNote.trim().length<5}
   ...
   ?<div className="wf-reject-form">
      ...
      <div className="wf-actions">
        ...
        <button className="wf-primary" disabled={fixNote.trim().length<5||!fixReady}
          onClick={()=>onAction("Submitted",fixNote.trim(),edit)}><Check/>Send back to {queriedByAudit?"Audit":"Accounts"}</button>
      </div>
    </div>
